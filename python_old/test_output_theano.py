#####################################################################
# Generates three arrays corresponding to max activations of nodes
# in network:
# - max_values: maximum values of nodes over testing period
# - max_states: states corresponding to max_values (with shape
#                [nodes, channels, res_y, res_x])
# - max_positions: positions corresponding to max_values (with shape
#                  [nodes, 4], where columns represent [time, x, y, z])
#
# This is a rough implementation with lots of hard coding.
#####################################################################
...
def create_network(available_actions_count):
    ...
    dqn = InputLayer(shape=[None, 3, resolution[0], resolution[1]], input_var=s1)
    ...
    function_get_fc_output = theano.function([s1], fc_output, name="fc_out_fn")
    print("Network compiled.")

    def simple_get_best_action(state):
        return function_get_best_action(state.reshape([1, 3, resolution[0], resolution[1]]))
    
    def simple_get_fc_output(state):
        return function_get_fc_output(state.reshape([1, 3, resolution[0], resolution[1]]))

    # Returns Theano objects for the net and functions.
    return dqn, function_get_q_values, simple_get_best_action, simple_get_fc_output
...
net, get_q_values, get_best_action, get_fc_output = create_network(len(actions))
...
# Create arrays to store max activated states
...
for test_episode in range(test_episodes):
    ...
    while not game.is_episode_finished():
        print("Episode time: %i" % game.get_episode_time())
        sleep(0.001)
        ...
        # Obtain fc layer output and store if max activated
        # Don't have to worry about negatives because of ReLU
        fc_output = get_fc_output(state)
        #heatmap = plt.pcolor(fc_output)
        #plt.colorbar(heatmap)
        #plt.show()
        max_mask = (fc_output > np.amin(max_values, axis=1))
        max_mask = np.reshape(max_mask, [4608,1])
        idx = np.argmin(max_values, axis=1)
        max_values[np.arange(4608), idx] = np.where(max_mask.reshape([4608,]),
                                                    fc_output,
                                                    max_values[np.arange(4608), idx])
        max_states[np.arange(4608), idx] = np.where(max_mask[:, :, np.newaxis, np.newaxis],
                                                    state[np.newaxis, :, :, :],
                                                    max_states[np.arange(4608), idx])
        pos = agent.get_current_position()
        max_positions[np.arange(4608), idx] = np.where(max_mask,
                                                       pos[np.newaxis, :],
                                                       max_positions[np.arange(4608), idx])
