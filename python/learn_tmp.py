...
for epoch in range(epochs):
    ...
    for test_episode in range(test_episodes_per_epoch):
        ...
        while not game.is_episode_finished():
            ...
            feed_dict = {agent.network.state: agent.state[np.newaxis, :], agent.network.position: [[pos_x, pos_y]], 
                         agent.network.IS_weights: weights}
            loss += agent.network.sess.run(agent.network.loss, feed_dict=feed_dict)
            a = randint(0, 2)
