...
from lasagne.layers import Conv2DLayer, InputLayer, DenseLayer, get_output, \
    get_all_params, get_all_param_values, set_all_param_values, get_all_layers, \
    get_output_shape
...
class ReplayMemory:
    def __init__(self, capacity):
        state_shape = (capacity, 3, resolution[0], resolution[1])
    ...
    def get_sample(self, sample_size):
        ...
        for i in idx:
            ...
            a.append(self.a[i])
            s2.append(self.s2[i])
            isterminal.append(self.isterminal[i])
            r.append(self.r[i])
            j += 1
        return np.asarray(s1), np.asarray(a), np.asarray(s2), np.asarray(isterminal), np.asarray(r)
...
def perform_learning_step(epoch):
    ...
    #plt.imshow(np.transpose(s1, [1,2,0])[:,:,[2,1,0]])
    #plt.show()
    #sleep(1)
