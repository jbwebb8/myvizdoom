...
# Note:
#   test switching order of batch files to insert try-except clause
###############################################################################
...
parser.add_argument("-b", "--batch_files", default=None, metavar="", nargs="*",
                    help="data file(s) containing training batch(es)")
parser.add_argument("--preprocess", action="store_true", default=False, 
                    help="preprocess batch files")
...
#    pass
batch_file_paths = args.batch_files
...
def save_exp_details(folder, agent):
    ...
    f.write("Batch files: " + ", ".join(batch_file_paths) + "\n")
    ...
    files_to_copy = [agent_file_path, net_file_path]
...
# Load batch files
print("Processing training batches...", end=""), sys.stdout.flush()
batches = []
for bf in batch_file_paths:
    batches.append(np.load(bf))
if preprocess:
    batches = agent.preprocess_batch(*batches)
print("Done.")
...
for epoch in range(epochs):
    ...
    for learning_step in trange(learning_steps_per_epoch):
        agent.learn_from_batch(*batches)
